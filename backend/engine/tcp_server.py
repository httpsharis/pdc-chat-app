...
def handle_client(client_socket, addr):
    ...
    try:
            ...
            if command.startswith("CONNECT "):
                ...
                with clients_lock:
                    if proposed_username in clients:
                        ...
                        try:
                            old_sock.send(
                                b"ERROR: Session taken over by another location.\n")
                            old_sock.close()
                        except Exception:
                            pass
                    username = proposed_username
                    clients[username] = {
                        "socket": client_socket, "last_active": time.time()}
                ...
                if current_room and username:
                    ...
                    room_manager.broadcast_to_room(
                        current_room, formatted_msg, client_socket)
                ...
                with clients_lock:
                    if target_user in clients:
                        ...
                        try:
                            target_sock.send(
                                b"You have been kicked by an admin.\n")
                            target_sock.close()
                        except Exception:
                            pass
                        client_socket.send(
                            f"SUCCESS: {target_user} kicked.\n".encode('utf-8'))
                    else:
                        client_socket.send(
                            f"ERROR: {target_user} not found.\n".encode('utf-8'))
                ...
                if username:
                    target_room = command[6:].strip()
                    if target_room:
                        # Broadcast transient event to everyone EXCEPT the sender
                        typing_event = f"TYPING_EVENT:{target_room}:{username}\n"
                        room_manager.broadcast_to_room(
                            target_room, typing_event, client_socket)
        ...
        if username:
            with clients_lock:
                # CRITICAL FIX: Only delete the user from the dictionary
                # if the socket currently registered to that username is
                # THIS EXACT socket. Prevents new connections from being
                # deleted by old dying threads.
                if username in clients and clients[username]["socket"] == client_socket:
